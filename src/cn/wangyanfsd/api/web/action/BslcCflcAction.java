...
public class BslcCflcAction extends HttpServlet {

    private BslcCflcService bslcCflcService = (BslcCflcService)new TransactionUtil().newProxy(new BslcCflcServiceImpl());
    ...
    private Result save() {
        ...
        if(!"".equals(title)&&title!=null&&!"".equals(content)&&content!=null&&!"".equals(date)&&date!=null&&!"".equals(person)&&person!=null){
            BslcCflc bslcCflc = new BslcCflc();
            bslcCflc.setTitle(title);
            bslcCflc.setContent(content);
            bslcCflc.setDate(date);
            bslcCflc.setPerson(person);
            bslcCflcService.save(bslcCflc);
            map.put("bslcCflc", bslcCflc);
    ...
    private Result remove() {
        ...
        if(!"".equals(id)&&id!=null&&StrUtil.isNumeric(id)){
            ...
            BslcCflc bslcCflc = bslcCflcService.find(i);
            FileTools.rm(request.getSession().getServletContext().getRealPath("") + "/WEB-INF/upload/",bslcCflc.getContent());
            map.put("bslcCflc", bslcCflcService.find(i));
            bslcCflcService.remove(i);
    ...
    private Result update() {
        ...
        if(!"".equals(id)&&id!=null&&StrUtil.isNumeric(id)&&!"".equals(title)&&title!=null&&!"".equals(content)&&content!=null&&!"".equals(date)&&date!=null&&!"".equals(person)&&person!=null){
            BslcCflc bslcCflc = new BslcCflc();
            bslcCflc.setId(Integer.parseInt(id));
            bslcCflc.setTitle(title);
            bslcCflc.setContent(content);
            bslcCflc.setDate(date);
            bslcCflc.setPerson(person);
    ...
    private Result list() {
    	...
    	LiuLiang ll = liuLiangService.find("content = 'BslcCflc' ",null);
		...
		List <BslcCflc> result;

        if(!"".equals(where)&&where!=null){
        	result = bslcCflcService.list(" title LIKE '%"+where+"%' OR content LIKE '%"+where+"%'  ",null);
        	for (BslcCflc bslcCflc : result) {
        		bslcCflc.setContent(basePath+bslcCflc.getContent());
			}
            message = JSONUtils.toJSONString(result);
        }else{
        	result = bslcCflcService.list();
        	for (BslcCflc bslcCflc : result) {
        		bslcCflc.setContent(basePath+bslcCflc.getContent());
			}
            message = JSONUtils.toJSONString(result);
    ...
    private Result find() {
        ...
        if(!"".equals(id)&&id!=null&&StrUtil.isNumeric(id)){
            int i = Integer.parseInt(id);
            message = JSONUtils.toJSONString(bslcCflcService.find(i));
        }
